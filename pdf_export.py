...
def _validate_friendly_plan_distribution(
    ...
    if overlap_count > 0:
        raise ValueError("Validación de planes falló: hay soluciones repetidas entre planes.")

    for row in short_rows:
        if _canonical_horizon(str(row.get("plazo", ""))) != "Corto plazo":
            raise ValueError("Validación de planes falló: el plan de corto plazo contiene soluciones de otra etapa.")
    for row in medium_rows:
        if _canonical_horizon(str(row.get("plazo", ""))) != "Mediano plazo":
            raise ValueError("Validación de planes falló: el plan de mediano plazo contiene soluciones de otra etapa.")
    for row in long_rows:
        if _canonical_horizon(str(row.get("plazo", ""))) != "Largo plazo":
            raise ValueError("Validación de planes falló: el plan de largo plazo contiene soluciones de otra etapa.")

    if quick_win_keys & plan_union:
        raise ValueError("Validación de planes falló: una acción de impacto inmediato fue incluida en los planes.")
    ...
    if missing or extra:
        detail = []
        if missing:
            detail.append(f"huérfanas={len(missing)}")
        if extra:
            detail.append(f"extra={len(extra)}")
        raise ValueError("Validación de planes falló: distribución inconsistente (" + ", ".join(detail) + ").")
