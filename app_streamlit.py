...
def _send_statistical_csv_via_gmail(
    *,
    ...
    msg = EmailMessage()
    msg["Subject"] = f"RoGen statistical record {case_id}"
    msg["From"] = cfg.smtp_from
    msg["To"] = recipient
    msg.set_content(
        "Attached is the internal statistical CSV generated by RoGen. "
        "This message is for research traceability only and is not sent to the evaluated company.\n\n"
        f"Case ID: {case_id}\n"
        f"Generated at: {record.get('generated_at', '')}\n"
        f"Company size: {record.get('company_size', '')}\n"
        f"Maturity gap: {record.get('maturity_gap', '')}\n"
        f"Recommendations: {record.get('recommendations_total', '')}\n"
        f"Budget fit ratio: {record.get('budget_fit_ratio', '')}\n"
    )
    msg.add_attachment(
        csv_bytes,
        maintype="text",
        subtype="csv",
        filename=f"rogen_statistical_record_{case_id}.csv",
    )
        ...
        try:
            ...
            with st.spinner(_t(language, "generating")):
                ...
                try:
                    ...
                    stats_csv = build_statistical_csv_bytes(stats_record)
